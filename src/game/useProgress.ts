...
export function useProgress() {
  ...
  const unlockedThemeIds = useMemo(
    () => evaluateThemeUnlocks(themeContext),
    [themeContext],
  );

  const activeTheme = useMemo(() => {
    const selected = themeById(snapshot.profile.selectedTheme);
    return isThemeUnlocked(selected, themeContext)
      ? selected
  ...
  const markModeIntroSeen = useCallback(
    (mode: GameMode) => {
      setSnapshot((prev) => {
  ...
  const recordSession = useCallback(
    (result: GameSessionResult): SessionSummary | null => {
      ...
      setSnapshot((prev) => {
        ...
        /* ---- statistics ---- */
        const reactionTotal =
          avg !== null ? avg * (stats.perfect + stats.fast + stats.good) : 0;
        ...
        const nextMode = {
          ...
          totalSuccessfulReactions:
            prevMode.totalSuccessfulReactions + stats.successes,
          ...
          fastestReaction: betterLow(
            prevMode.fastestReaction,
            stats.fastestReaction,
          ),
          bestAvgReaction:
            samples >= 5
              ? betterLow(prevMode.bestAvgReaction, avg)
              : prevMode.bestAvgReaction,
        ...
        const statistics = {
          ...
          totalSuccessfulReactions:
            prev.statistics.totalSuccessfulReactions + stats.successes,
          ...
          fastestReaction: betterLow(
            prev.statistics.fastestReaction,
            stats.fastestReaction,
          ),
          ...
          totalTrapsAvoided:
            prev.statistics.totalTrapsAvoided + stats.trapsAvoided,
          totalPurpleCompletions:
            prev.statistics.totalPurpleCompletions + stats.purpleCompletions,
          totalPlayTime:
            prev.statistics.totalPlayTime + Math.max(0, result.durationMs),
        ...
        if (
          ...
          (records.fastestReaction === null ||
            stats.fastestReaction < records.fastestReaction)
        ) {
        ...
        if (runValue !== null) {
          const progress = mergeChallengeProgress(
            todaysChallenge,
            daily.progress,
            runValue,
          );
        ...
        const unlockedThemes = afterThemes.filter(
          (id) => !beforeThemes.includes(id),
        );
